...
def validate(
    ...
    ref_df = (
        pd.read_csv(train_reference)
        if train_reference and train_reference.exists()
        else None
    )
    ...
    report = {
        ...
        "validity_ratio": (
            round(stats["validated"] / stats["total"], 2) if stats["total"] else 0
        ),
        "magnetic_ratio": (
            round(stats["magnetic_count"] / stats["validated"], 2)
            if stats["validated"]
            else 0
        ),
        "novelty_ratio": (
            round(stats["novel_count"] / stats["validated"], 2)
            if stats["validated"]
            else 0
        ),
        "duplicate_ratio": (
            round(reasons_count.get("duplicate", 0) / stats["total"], 2)
            if stats["total"]
            else 0
        ),
        "avg_density": (
            round(sum(valid_densities) / len(valid_densities), 2)
            if valid_densities
            else 0
        ),
        "avg_volume_per_atom": (
            round(sum(valid_vpas) / len(valid_vpas), 2) if valid_vpas else 0
        ),
    ...
    print(
        f"[bold green]Валидация завершена![/bold green] Успешно: {stats['validated']}"
    )
